    ...
    async execute(client, interaction) {
        ...
        if (timestamps.has(interaction.user.id)) {
            ...
            if (now < expirationTime) {
                ...
                return interaction.reply(`Подождите **${timeLeft.toFixed(2)}s.** и используйте команду ещё раз.`, true);
            }
        }
        ...
        try { cmd.execute(client, interaction) } catch { interaction.reply({ content: `> ${client.emoji.danger} Произошла ошибка.`, ephemeral: true }) }
    }
}
